...
public class ThemeSettingValidator implements AccountSettingValidator {
    private static final List<String> ALLOWED = List.of("light", "dark", "system");

    @Override
    public AccountSettingsKeyEnum getKey() {
        return AccountSettingsKeyEnum.S_THEME;
    }
    ...
    public void validate(String value) throws ExceptionInvalidAccountSettingValue {
        if (!getAllowedValues().contains(value)) {
            throw new ExceptionInvalidAccountSettingValue(
                    EnumExceptionName.ERROR_VALIDATION_INVALID_SETTINGS_THEME.name(), new Object[] { value }
            );
        }
    }
}